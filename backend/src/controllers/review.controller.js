...
async function getReviewsByDestination(req, res, next) {
    try {
        res.json(
            await reviewService.getReviewsByDestination(
                req.params.id,
                req.query,
            ),
        );
...
async function getReviewsByTour(req, res, next) {
    try {
        res.json(
            await reviewService.getReviewsByTour(req.params.id, req.query),
        );
    } catch (error) {
        next(error);
    }
...
// Hàm updateReview: người dùng sửa đánh giá cho điểm đến hoặc tour.
async function updateReview(req, res, next) {
    try {
        res.status(201).json(
            await reviewService.updateReview(req.params.id, req.user, req.body),
        );
    } catch (error) {
        next(error);
    }
